...
function GroupRankPage(){
  ...
  return(
    ...
    <div className='flex flex-row gap-6 mt-6 justify-between'>
      <h1 className='font-semibold text-3xl'>{`그룹 ${rankType === 'score' ? '점수' : rankType === 'streak' ? '최장 스트릭' : '푼 문제 수'} 랭킹`}</h1>
      {rankType && <div role="tablist" className="w-fit tabs tabs-box">
        ...
        <Link to="/rank/groups/count" role="tab" className={`tab ${rankType === 'count' && 'tab-active'}`}>그룹 해결 문제</Link>
        <Link to="/rank/users/score" role="tab" className="tab">유저 점수</Link>
        <Link to="/rank/users/streak" role="tab" className="tab">유저 스트릭</Link>
        <Link to="/rank/users/count" role="tab" className="tab">유저 해결 문제</Link>
      </div>}
    </div>
    { data?.success === true &&
      <RankTable rankType={rankType} datas={{type:'group', groupDatas:data.result}}>
      </RankTable>
    }
    </>
  )
